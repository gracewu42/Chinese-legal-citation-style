...
var noteIndexFromCluster = function(cluster) {
	return cluster.properties.noteIndex;
};
	...
	var addCiteprocChange = function (newCluster) {
		...
		if (originalIndex + 1 < previousClusterList.clusters.length) {
			citationsPost = _.map(_.range(originalIndex + 1, previousClusterList.clusters.length), function (index) {
				return [previousClusterList.clusters[index].cluster.citationID, noteIndexFromCluster(previousClusterList.clusters[index].cluster)];
			});
		}
		...
		citationsPre.push([newCluster.citationID, noteIndexFromCluster(newCluster)]);
		...
		switch (diff.action) {
			case "equal":
				_.each(diff.items, function (clusterString) {
					...
					var footnoteIndex = noteIndexFromCluster(previousClusterList.clusters[originalIndex].cluster);
					that.clusters[currentIndex].cluster.citationID = citationID;
					citationsPre.push([citationID, footnoteIndex]);
				});
				break;
			case "remove":
...
var connectToMendeleyDesktop = function () {
    if (typeof qt == 'undefined') throw "Qt not found!";
...
var processCitations = function (style, locale, citationClusters, items, enumerateCitations, reset, bibliographyOutputFormat) {
	...
	try {
			...
			Mendeley_citeprocReact = new CiteprocReact({
				citeprocSys: {
					...
					getAbbreviation: function (dummy, obj, jurisdiction, vartype, key) {
						if (vartype == "container-title" && vartype in obj["default"]) {
							var abbreviated = getValueFromMap(journalAbbreviations, key, key);
							obj[jurisdiction][vartype][key] = abbreviated;
						}
						else {
							// citeproc-js expects the obj object to have
							// container-title, title, publisher...
							// title was added in citeproc-js 1.1.153
							// so here we add any possible vartype for the future.
							// If Mendeley Desktop needs to support abbreviations for
							// other fields than container-title this needs to be modified.
							if (typeof (obj[jurisdiction]) === "undefined") {
								obj[jurisdiction] = Object;
							}
							if (typeof (obj[jurisdiction][vartype]) === "undefined") {
								obj[jurisdiction][vartype] = Object;
							}
						}
					},
		...
		if (citationClusters.length === 0 && !result.formattedBibliography && _.keys(items).length > 0) {
			...
			// bibliography (this is used when a Mendeley Desktop user selects View -> Library as Citations).
			//
			// The _.keys(items).length > 0 is needed for example when a user is refreshing
			// a Word/LibreOffice document without any citations.
